#!/usr/bin/env bash
# 05-create-llm-vm.sh – скрипт создания и настройки VM для LLM
# Подробные комментарии на русском языке поясняют каждую часть процесса
# Скрипт поддерживает идемпотентный запуск: если VM уже существует, обновляет конфигурацию без удалений
